  ...
  if(p=='+')
    {
      ...
      printf("The %d-th movement is from track %d to %d with total seek of: %d\n",++j,size,queue[0],abs(size-queue[0]));  // Circular movement from end to beginning query
      for(i=1;i<z;i++)
      {
        printf("The %d-th movement is from track %d to %d with total seek of: %d\n",j+1,queue[i-1],queue[i],abs(queue[i]-queue[i-1]));
        j++;s+=abs(queue[i]-queue[i-1]);
    ...
    else
    {
      ...
      if(queue[n-1]!=size){printf("The %d-th movement is from track %d to %d with total seek of: %d\n",++j,size,queue[n-1],abs(size-queue[n-1]));s+=abs(size-queue[n-1]);}
      for(i=n-1;i>z;i--)
      {
        printf("The %d-th movement is from track %d to %d with total seek of: %d\n",j+1,queue[i],queue[i-1],abs(queue[i]-queue[i-1]));
        j++;s+=abs(queue[i]-queue[i-1]);
...
The 8-th movement is from track 199 to 14 with total seek of: 185
The 9-th movement is from track 14 to 37 with total seek of: 23

Final seek amount: 169
...
The 4-th movement is from track 199 to 183 with total seek of: 16
